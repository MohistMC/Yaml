...
public class CommentEventsCollector {
  ...
  private List<com.mohistmc.snakeyaml.comments.CommentLine> commentLineList;
  private final Queue<Event> eventSource;
  private final com.mohistmc.snakeyaml.comments.CommentType[] expectedCommentTypes;
   ...
   *        collected.
   */
  public CommentEventsCollector(final Parser parser, com.mohistmc.snakeyaml.comments.CommentType... expectedCommentTypes) {
   ...
   *        collected.
   */
  public CommentEventsCollector(Queue<Event> eventSource, com.mohistmc.snakeyaml.comments.CommentType... expectedCommentTypes) {
  ...
  public Event collectEvents(Event event) {
    if (event != null) {
      if (isEventExpected(event)) {
        commentLineList.add(new com.mohistmc.snakeyaml.comments.CommentLine((CommentEvent) event));
      } else {
        return event;
      }
    }
    while (isEventExpected(eventSource.peek())) {
      commentLineList.add(new com.mohistmc.snakeyaml.comments.CommentLine((CommentEvent) eventSource.poll()));
    }
    return null;
  }
